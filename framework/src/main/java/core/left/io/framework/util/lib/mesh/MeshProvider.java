...
public class MeshProvider implements MeshStateListener {
    ...
    public List<MeshPeer> getLivePeers() {

        List<MeshPeer> meshPeerList = new ArrayList<>();

        synchronized (mMeshIdSet) {

            for (byte[] meshId : mMeshIdSet) {
                if (meshId != null) {
                    meshPeerList.add(new MeshPeer(meshId));
                }
            }
    ...
    private int send(MeshData meshData) throws RightMeshException {
        ...
        MeshId meshId = new MeshId(meshData.mMeshPeer.getPeerId());
    ...
    public void meshStateChanged(MeshId meshId, int state) {
        ...
        if (state == MeshStateListener.SUCCESS || state == MeshStateListener.RESUME) {
            ...
            if(mIMeshCallBack != null) {
                ...
                mIMeshCallBack.onInitSuccess(new MeshPeer(meshId.getRawMeshId()));
    ...
    private void handlePeerChanged(MeshManager.RightMeshEvent e) {
        ...
        if(e != null) {
            ...
            if (event.peerUuid != null && (event.state == MeshManager.REMOVED ||
                    event.state == MeshManager.ADDED || event.state == MeshManager.UPDATED)) {

                byte[] meshPeerId = event.peerUuid.getRawMeshId();
                ...
                if(event.state == MeshManager.ADDED) {
                    ...
                    if(mMeshIdSet.add(meshPeerId)) {//If it was really able to add
                        ProfileManager.getInstance().sendMyProfileInfo(new MeshPeer(event.peerUuid.getRawMeshId()));
                    }
                    ...
                    mMeshIdSet.remove(meshPeerId);//remove record
    ...
    private void handleDataReceived(MeshManager.RightMeshEvent e) {
        ...
        if(meshData != null) {
            ...
            if (profileData == null) {//not profile data
                ...
                if(isPeerAvailable(dataReceivedEvent.peerUuid.getRawMeshId())) {
                    meshData.mMeshPeer = new MeshPeer(dataReceivedEvent.peerUuid.getRawMeshId());
    ...
    private boolean isPeerAvailable(byte[] targetPeer) {

        String targetedPeer = Arrays.toString(targetPeer);
        if(!TextUtils.isEmpty(targetedPeer)) {

            synchronized (mMeshIdSet) {
                for (byte[] peer : mMeshIdSet) {
                    if (targetedPeer.equals(Arrays.toString(peer))) {
                        return true;
                    }
                }
            }
